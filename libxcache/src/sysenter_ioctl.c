...
#undef X
  ...
  case CALL_SYSCONF:
    if (ERROR((rc = libc_sysconf(inf, thread))))
      goto done;
    break;

  case CALL_GETENV:
    if (ERROR((rc = libc_getenv(inf, thread))))
      goto done;
    break;

  case CALL_SETENV:
    if (ERROR((rc = libc_setenv(inf, thread))))
      goto done;
    break;
  ...
  case CALL_PUTENV:
    if (ERROR((rc = libc_putenv(inf, thread))))
      goto done;
    break;

  case CALL_CLEARENV:
    if (ERROR((rc = libc_clearenv(inf, thread))))
      goto done;
    break;
