...
int see_newfstatat(tracee_t *tracee, int dirfd, const char *pathname) {
  ...
  if (dirfd == AT_FDCWD) {
    ...
    DEBUG("PID %d called newfstatat(%d, \"%s\", …)", (int)tracee->pid, dirfd,
          pathname);
  }
  ...
  if (pathname[0] == '/') {
    ...
    // was this newfstatat call passed an invalid handle?
    if (of == NULL)
      goto done;
